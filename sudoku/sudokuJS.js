    ...
    $.fn.sudokuJS = function(opts) {
		...
		*/
		...
		var clearBoard = function(){
			...
			$boardInputs
				.removeClass("highlight-val")
				.removeClass("init-highlight-val")
				.val("");
		 ...
		 * in one go. Since it also only updates candidates, we can skip straight to next strat, 
		 * since we know that neither this one nor the one(s) before (that only look at actual numbers on board),
		 * will find anything new.
		 * -----------------------------------------------------------------*/
		...
		var keyboardMoveBoardFocus = function(currentId, keyCode){
			...
			do {
				...
				if(newId < 0 || newId >= (boardSize*boardSize)){
					if(newId < 0)newId=boardSize*boardSize-1;
					else if(newId >= (boardSize*boardSize))newId=0;
				}
				//console.log("newId: "+newId);
			} while($("#input-"+newId).hasClass('init-highlight-val'))

			//TODO class '.highlight-val' from previous cell
			$("#input-"+currentId).removeClass('.highlight-val');
		...
		* -----------------------------------------------------------------*/
		...
		$boardInputs.on("mouseup", function(e){
			var $this = $(this);
			var id = parseInt($this.attr("id").replace("input-",""));
			selectedIndex = id;
			//console.log("mouseup selected index: "+selectedIndex);
		...
		$boardInputs.on("keyup", function(e){
			...
			var id = parseInt($this.attr("id").replace("input-",""));
			selectedIndex = id;
			//console.log("keyup selected index: "+selectedIndex);
			//allow keyboard movements
			if(e.keyCode >=37 && e.keyCode <= 40){// || e.keyCode ===48){
				keyboardMoveBoardFocus(id, e.keyCode);
			}
		...
		* ----------------- */
		...
		return {
			...
			//added, to return the selectedIndex
			getSelectedIndex : getSelectedIndex,
			keyboardNumberInput : keyboardNumberInput,
			renderBoardCell : renderBoardCell,
		};
	};
