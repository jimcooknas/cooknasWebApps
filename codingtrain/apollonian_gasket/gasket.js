...
class Gasket {
  constructor(x, y, r, color, seed) {
    randomSeed(seed);
    this.allCircles = [];
    this.queue = [];
    let c1 = new Circle(-1 / r, x, y, index);
    let r2 = random(c1.radius / 4, c1.radius / 2);//c1.radius * 0.25;
    //console.log(c1);
    let v = p5.Vector.fromAngle(-PI/2);//random(TWO_PI));
    v.setMag(c1.radius - r2);
    let c2 = new Circle(1 / r2, x + v.x, y + v.y, index);
    let r3 = v.mag();
    v.rotate(PI);
