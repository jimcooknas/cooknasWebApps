...
function Schroedinger(settings){
	...
	this.saveDataToFile = false;
	this.saveImageToFile = false;
	...
	this.appendDataTable = function(){
		...
		if(this.frameCount === this.maxFrames && this.saveDataToFile ===true && this.dataFile !== null){
	...
	this.show = function(){
		...
		if(this.saveImageToFile===true && this.imageFile !== null){
	...
	this.plotComplexData = function(data, plotMap, plotHeight, topMargin, scaleFactor){
		...
		for(let j = 1; j < width-1; j++){
			...
			if(this.gradientColor)
				cl = Math.floor((val.arg()/Math.PI + 1)*128);
	...
	this.drawOverlay = function(seperatorHeight, textHeight){
		...
		text(' step = '+this.frameCount, width - 0.25*textHeight, 0.25*textHeight+20);
		colorMode(RGB, 255);
		stroke(0);
		fill(255, 255, 50);
		textSize(textHeight+4);
		text(this.label, width - 0.25*(textHeight+4), seperatorHeight + 0.25*textHeight);
	}
	...
	this.simulationStep = function(){
		...
		if(this.frameCount > this.maxFrames){ 
			noLoop();
			if(this.saveDataToFile === true && this.dataFile !== null){
